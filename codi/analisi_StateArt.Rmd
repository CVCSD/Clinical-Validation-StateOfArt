---
title: Estat de l'art sobre la validació clínica de solucions digitals en l’àmbit
  de la salut
...
A partir dels articles trobats en la cerca inicial (sense filtrar per any de publicació; n=1250 papers) es va realitzar un estudi bibliomètric. Es va crear un dataset recopilant informació relativa a cada article: any de publicació, revista, país de la revista, abstract, keywords, factor d'impacte, quartils i categoríes. Es va realitzar un anàlisi descriptiva de freqüències de les variables generades, anàlisi gràfic, Wordcloud de keywords y termes mesh, i una analisis de frequencies segons la regió i país del primer autor de l’article. Les variables relatives al tipus de disseny utilitzades i dominis es van generar mitjançant cerques de paraules en el títol, resum, i keywords. La relació de paraules sinonimes utilitzades segons disseny i domini es poden veure en les taules X de l’annex X. A més a més es va capturar l’àmbit d’aplicació i el quartil de la revista. 
Es va realitzar una taula descriptiva bivariada dels termes relatius al domini en relació al disseny de l'estudi (Observacional / Experimental). 
La gestió i anàlisi de dades es va realitzar mitjançant @R-base, les taules bivariades es van generar mitjançant el paquet @R-compareGroups, @compareGroups2014 i l'analisis gràfic es va realitzar mitjançant els paquets de R @R-ggplot2, @R-wordcloud2 i @R-maps. 
...
dades<-read.csv2(here::here("dades","papers.txt"), sep="#") %>% mutate(PMID=as.character(PMID))

dades_extra<-read.csv2(here::here("dades","objects.txt"), sep="#",encoding = "UTF-8")


dt_cataleg_countries<-read.csv2(here::here("dades","Countries.txt"), sep=",",encoding = "UTF-8")
...
path_docs<-here::here("resultats","taules.docx")
...
```{r lectura_clinicaltrials, include=FALSE}
...
- Llistat de termes / sinonims buscats per dominis/dissenys
...
taula1 %>% 
  kableExtra::kable(caption = "Tabla S1. Llistat de termes en Abstract keywords titul relatius a dissenys") %>% kableExtra::kable_classic_2()
...
#
taula2 %>% 
  kableExtra::kable(caption = "Tabla S2. Llistat de termes en Abstract/Keywords/Titol segons Dimensió o ámbit d'aplicació") %>% 
...
tt<-taula1 %>% 
  ...
  flextable::set_caption("Tabla S1. Llistat de termes en Abstract/Keywords/Titol segons Dimensió o ámbit d'aplicació") %>% 
  flextable::autofit()
...
tt<-taula2 %>% 
  flextable() %>% 
  flextable::theme_zebra() %>% 
  set_caption("Tabla S2. Llistat de termes en Abstract/Keywords/Titol segons Dimensió o ámbit d'aplicació") %>% 
  autofit()
...
La cerca va proporcionar un total de 1250 papers. En el Wordcloud es pot apreciar que les keywords més frequents fan referencia a mobile health , ehealth i validation. Del total dels 1250 papers es van identificar els dissenys epidemiologics de 449 d'ells, dels quals el 40% es van classificar com a clinical Trials, mentre la resta eren estudis observacionals o systematic reviews. Dels treballs que es va identificar el diseny epidemiologic, el 55.3% eren observacionals. 

De totes les referencies trobades, la dimensió mes frequent va ser la de Usability (n=222 17.8%), seguida de segurity (14.2%), i la de feasibility (10.0%). 

La taula X (Bivariada) / Figura, mostra la frequencia de papers trobats per dimensions en funció de si els treballs s'ha pogut classificar com observacionals o experimentals. S'observen diferencies significatives (p<0.05) en les dimensions de Feasibility (Prova de concepte), i Effectiveness, amb major representació els treballs experimentals en comparació amb els estudis observacionals. La resta de dimensions mostren frequencies similars, excepte la dimensió de Efficacy on també s'observa més representació en els Experimentals (10.6% versus 5.8%) 


Pel que fa a l'ámbit o scope s'aplicació on es publiquen, un 21.5% dels articles es van trobar en Health care sciences, i un 19% en revistes relatives a medical informatics.  La frequencia de papers ha aumentat de de manera exponencial sent sempre més frequent en revistes classificades en quartils 1 o 2.

Els autors que més publiquen son principalment del Estats units (25%), seguits de paisos europeus , com UK (7.53%), espanya, Germany i australia (7.53%).


## Limitacions sobre la interpretació

L'estudi bibliomètric es basa en la cerca de paraules de manera automàtica, fet que comporta certes limitacions intrínseques que cal tenir en compte en la interpretació. Això inclou la manca de context, l'evolució del llenguatge, els sinònims i les variacions lingüístiques que poden fer servir diferents paraules o frases per descriure conceptes semblants, la qual cosa pot conduir a una subrepresentació o sobrerepresentació de les dimensions i els dissenys identificats.
...
![Wordcloud Keywords](wordcloud.png){width=400% height=400% }
...
# table(dt_temp$Design_tipo)
# table(dt_temp$study_type)
# 
# 
# descrTable(~Design + Design_tipo + study_type + Prospective + Retrospective,method = 3,  data=dt_temp)
...
# 
# dades<- dades %>% mutate(Design_tipo = 
#            case_when(study_type=="Experimental" ~ "Clinical trial",
#            TRUE ~ study_type))
# 
# descrTable(~Design + Design_tipo + study_type + Prospective + Retrospective,method = 3,  data=dt_temp)
# 
...
# descrTable( formu , method = 3, data=dades, show.n = T, byrow = F, hide = "No") %>% export2md()
# 
...
```{r generaggplowchart1, eval=FALSE}
...
ggsave(here::here("resultats","fig2B.png"), fig3)

ggsave(here::here("resultats","fig2A.png"), fig4)

ggsave(here::here("resultats","fig3A.png"),gg_map2)

ggsave(here::here("resultats","fig3A_regions.png"),gg_map1)

htmlwidgets::saveWidget(world_cloud_keywords, "wordcloud.html", selfcontained = F)
webshot::webshot("wordcloud.html", file=here::here("resultats","wordcloud.png"), delay = 5, vwidth = 1000, vheight = 1000)
